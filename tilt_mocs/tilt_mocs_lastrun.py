...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_15 = visual.TextStim(win=win, name='text_15',
        text='for example, this one is pointing right, so you would press the period key',
        font='Arial',
    ...
    # --- Initialize components for Routine "practice_feedback_timed" ---
    ...
    # --- Prepare to start Routine "instr_example" ---
    ...
    center_practice_example.setSF(center_sf)
    center_practice_example.setPhase(0.0)
    # store start times for instr_example
    instr_example.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    instr_example.tStart = globalClock.getTime(format='float')
    instr_example.status = STARTED
    thisExp.addData('instr_example.started', instr_example.tStart)
    instr_example.maxDuration = None
    # keep track of which components have finished
    instr_exampleComponents = instr_example.components
    for thisComponent in instr_example.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "instr_example" ---
    instr_example.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        if key_resp_11.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_11.getKeys(keyList=['period'], ignoreKeys=["escape"], waitRelease=False)
        ...
        # if center_practice_example is starting this frame...
        if center_practice_example.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
            center_practice_example.frameNStart = frameN  # exact frame index
            center_practice_example.tStart = t  # local t and not account for scr refresh
            center_practice_example.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(center_practice_example, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'center_practice_example.started')
            # update status
            center_practice_example.status = STARTED
            center_practice_example.setAutoDraw(True)
        
        # if center_practice_example is active this frame...
        if center_practice_example.status == STARTED:
            # update params
            pass
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            instr_example.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in instr_example.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "instr_example" ---
    for thisComponent in instr_example.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for instr_example
    instr_example.tStop = globalClock.getTime(format='float')
    instr_example.tStopRefresh = tThisFlipGlobal
    thisExp.addData('instr_example.stopped', instr_example.tStop)
    # check responses
    ...
    thisExp.nextEntry()
    # the Routine "instr_example" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisPractice_no_time in practice_no_time:
        ...
        while continueRoutine:
            ...
            if key_resp_3.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_3.getKeys(keyList=['z','period'], ignoreKeys=["escape"], waitRelease=False)
    ...
    for thisPractice_timed in practice_timed:
        ...
        practice_trial_timed = data.Routine(
            ...
            components=[noise_2, surround_practice_2, center_practice_2, key_resp_4, text_12],
        )
        practice_trial_timed.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        noise_2.setOpacity(noise_opacity)
        surround_practice_2.setOpacity(orient_opacity)
        ...
        while continueRoutine:
            ...
            if noise_2.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if noise_2 is stopping this frame...
            if noise_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > noise_2.tStartRefresh + 0.08-frameTolerance:
                    # keep track of stop time/frame for later
                    noise_2.tStop = t  # not accounting for scr refresh
                    noise_2.tStopRefresh = tThisFlipGlobal  # on global time
                    noise_2.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'noise_2.stopped')
                    # update status
                    noise_2.status = FINISHED
            ...
            if key_resp_4.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_4.getKeys(keyList=['z','period'], ignoreKeys=["escape"], waitRelease=False)
        ...
        # --- Prepare to start Routine "practice_feedback_timed" ---
        # create an object to store info about Routine practice_feedback_timed
        practice_feedback_timed = data.Routine(
            name='practice_feedback_timed',
            components=[text_16],
        )
        practice_feedback_timed.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from code_7
        if key_resp_4.corr == True:
            msg = 'Correct'
        else:
            msg = 'Incorrect'
        text_16.setText(msg)
        # store start times for practice_feedback_timed
        practice_feedback_timed.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        practice_feedback_timed.tStart = globalClock.getTime(format='float')
        practice_feedback_timed.status = STARTED
        thisExp.addData('practice_feedback_timed.started', practice_feedback_timed.tStart)
        practice_feedback_timed.maxDuration = None
        # keep track of which components have finished
        practice_feedback_timedComponents = practice_feedback_timed.components
        for thisComponent in practice_feedback_timed.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "practice_feedback_timed" ---
        # if trial has changed, end Routine now
        if isinstance(practice_timed, data.TrialHandler2) and thisPractice_timed.thisN != practice_timed.thisTrial.thisN:
            continueRoutine = False
        practice_feedback_timed.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 1.5:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            if text_16.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > text_16.tStartRefresh + 1.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                practice_feedback_timed.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in practice_feedback_timed.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "practice_feedback_timed" ---
        for thisComponent in practice_feedback_timed.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for practice_feedback_timed
        practice_feedback_timed.tStop = globalClock.getTime(format='float')
        practice_feedback_timed.tStopRefresh = tThisFlipGlobal
        thisExp.addData('practice_feedback_timed.stopped', practice_feedback_timed.tStop)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if practice_feedback_timed.maxDurationReached:
            routineTimer.addTime(-practice_feedback_timed.maxDuration)
        elif practice_feedback_timed.forceEnded:
            routineTimer.reset()
        else:
            routineTimer.addTime(-1.500000)
        thisExp.nextEntry()
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[noise_3, surround_grating, center_grating, resp],
        )
        trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        noise_3.setOpacity(noise_opacity)
        # Run 'Begin Routine' code from set_phase
        center_phase = random.random()
        surround_phase = random.random()
        surround_grating.setOpacity(orient_opacity)
        ...
        while continueRoutine:
            ...
            # if noise_3 is starting this frame...
            if noise_3.status == NOT_STARTED and tThisFlip >= isi-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'noise_3.started')
                # update status
                noise_3.status = STARTED
                noise_3.setAutoDraw(True)
            
            # if noise_3 is active this frame...
            if noise_3.status == STARTED:
                # update params
                pass
            
            # if noise_3 is stopping this frame...
            if noise_3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > noise_3.tStartRefresh + 0.08-frameTolerance:
                    # keep track of stop time/frame for later
                    noise_3.tStop = t  # not accounting for scr refresh
                    noise_3.tStopRefresh = tThisFlipGlobal  # on global time
                    noise_3.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'noise_3.stopped')
                    # update status
                    noise_3.status = FINISHED
            ...
            if surround_grating.status == NOT_STARTED and tThisFlip >= isi-frameTolerance:
                # keep track of start time/frame for later
            ...
            if center_grating.status == NOT_STARTED and tThisFlip >= isi-frameTolerance:
                # keep track of start time/frame for later
            ...
            if resp.status == STARTED and not waitOnFlip:
                theseKeys = resp.getKeys(keyList=['z','period'], ignoreKeys=["escape"], waitRelease=False)
