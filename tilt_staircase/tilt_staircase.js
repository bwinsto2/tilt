...
let expName = 'tilt_staircase';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'tilt_staircase.csv', 'path': 'tilt_staircase.csv'},
...
function trialsLoopBegin(trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    trialsConditions = TrialHandler.importConditions(psychoJS.serverManager, 'tilt_staircase.csv');
...
function practice_trial_timedRoutineBegin(snapshot) {
  return async function () {
    ...
    practice_trial_timedComponents.push(text_10);
...
var _pj;
var surround_opacity;
var surround_orientation;
var _resp_allKeys;
...
function trialRoutineBegin(snapshot) {
  return async function () {
    ...
    _pj = {};
    _pj_snippets(_pj);
    psychoJS.experiment.addData("center_angle", level);
    surround_opacity = 100;
    if (_pj.in_es6("negs", label)) {
        surround_orientation = (- 30);
        psychoJS.experiment.addData("surround_angle", surround_orientation);
    } else {
        if (_pj.in_es6("poss", label)) {
            surround_orientation = 30;
            psychoJS.experiment.addData("surround_angle", surround_orientation);
        } else {
            if (_pj.in_es6("noss", label)) {
                surround_opacity = 0;
                surround_orientation = 69;
            }
        }
    }
    
    surround_grating.setOpacity(surround_opacity);
    surround_grating.setOri(surround_orientation);
    center_grating.setOri(level);
    resp.keys = undefined;
    resp.rt = undefined;
    _resp_allKeys = [];
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    // *text_3* updates
    if (t >= 0.58 && text_3.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
